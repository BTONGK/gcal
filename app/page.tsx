...
import { signLocation } from '@/lib/signing'
...
export default function Home() {
  const hasSigning = !!process.env.TAPNET_SIGNING_SECRET
    ...
    <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center px-6 text-center">
      ...
      <div className="space-y-2 w-full max-w-xs">
        {LOCATIONS.map(({ loc, label }) => {
          const sig = hasSigning ? signLocation(loc) : 'dev'
          return (
            <Link
              key={loc}
              href={`/tap?loc=${loc}&sig=${sig}`}
      ...
      <p className="text-white/20 text-xs mt-8">TapNet · tap the world around you</p>
    </main>
  )
}
